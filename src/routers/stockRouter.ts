...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const url = "https://gapi.shinhansec.com:8443/openapi/v1.0/ranking/rising";
    const headers = {
      apiKey: process.env.SHINHAN_APP_KEY, // 예시: 인증 토큰을 Bearer 스키마로 전달하는 경우
      rejectUnauthorized: false,
    };
    try {
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await stockService.findStockTransactionByUserId(
        req.user!.id,
        Number(size),
        Number(page)
      );
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await stockService.getStocksOrderByCap(
        Number(page),
        Number(size)
      );
      return res.status(200).json({
...
const searchMiddlewares = [
  query("name").notEmpty().withMessage("종목명은 무조건 있어야 합니다."),
  validateHandler,
];
// 주식 명칭으로 검색
router.get(
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const stockService2: IStockService = new StockService(
        AppDataSource.createQueryRunner()
      );
      const result = await stockService2.getMyStockInfo(
        req.params.code,
        req.user!.id
      );
      return res.status(200).json({
