...
router.get('/list/:communityId', nullalbeAuthenticate, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const communityId = Number(req.params.communityId);
        const result = await commentService.commentFindByCommunityId(communityId, req.user?.id);
        res.send(result);
    } catch (error) {
        next(error);
    }
...
router.post('/like', authenticate, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const result = req.user && (await commentService.commentLike(commentId, req.user.id, likeType));
        res.status(200).json({
            success: true,
            message: '좋아요 완료',
        });
...
router.post('/', authenticate, async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        req.user && (await commentService.commentSave(content, req.user.id, communityId));
        res.status(200).json({
            success: true,
            message: '댓글 입력 완료',
        });
