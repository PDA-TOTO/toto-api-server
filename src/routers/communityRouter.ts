...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result =
        req.user &&
        (await communityService.voteChange(req.user.id, communityId, voteType));
      return res.status(200).json({
        success: true,
        message: "투표 완료",
        result: result,
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const krxCode = req.params.krxCode;
      const result = await communityService.getNaverStockInfo(krxCode);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { list } = req.body;
      const result = await communityService.getNaverRisingStockInfo(list);
      return res.status(200).json(result);
    } catch (error) {
      next(error);
    }
