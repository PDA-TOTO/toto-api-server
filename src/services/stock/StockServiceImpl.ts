...
export class StockService implements IStockService {
  ...
  setQueryRunner(queryRunner: QueryRunner): void {
    ...
    this.stockTransactionRepository =
      queryRunner.manager.getRepository(StockTransaction);
    ...
    this.portfolioService = createService(
      queryRunner,
  ...
  async getRecentPrice(code: string): Promise<number> {
    ...
    if (!price)
      throw new ApplicationError(400, "해당 코드가 존재하지 않습니다.");
  ...
  async getFinanceByCode(code: string): Promise<FinanceResponse> {
    ...
    if (!stockCode)
      throw new ApplicationError(400, "해당 주식이 존재하지 않음");
  ...
  async findStockTransactionByUserId(
    ...
    const [stockTransactions, total] =
      await this.stockTransactionRepository.findAndCount({
        where: {
          portfolio: {
  ...
  async getInfoWithChart(
    ...
    if (bundleUnit === "YEAR") {
      const stockCharts = await this.priceRepository
        ...
        .select(
          `DATE_FORMAT(price.date, '%Y') as year, ceiling(avg(price.ePr)) as epr`
        )
    ...
    if (bundleUnit === "MONTH") {
      const stockCharts = await this.priceRepository
        .createQueryBuilder("price")
        .select(
          `DATE_FORMAT(price.date, '%Y-%m') as yymm, ceiling(avg(price.ePr)) as epr`
        )
        .where("code = :code", { code: code })
        .andWhere("date between :prev and now()", { prev: after })
        .orderBy("yymm", "DESC")
  ...
  @Transaction()
  async getStocksOrderByCap(
    page?: number,
    size?: number
  ): Promise<GetStockWithCapResponse> {
    if (!size) size = 7;
    if (!page) page = 1;
    ...
    const [stocks, total] = await this.financeRepository
      ...
      .innerJoin(
        lastDate,
        "lastDate",
        "finance.code = lastDate.code and finance.yymm = lastDate.last_date"
      )
  ...
  toFinanceResponse(price: number, finances: Finance[]): FinanceResponse {
    ...
    const finance: FinanceResponse = {
      ...
      incomeGrownthRate:
        ((recent.income - old.income) / Math.abs(old.income)) * 100,
      netincomeGrownthRate:
        ((recent.netincome - old.netincome) / Math.abs(old.netincome)) * 100,
